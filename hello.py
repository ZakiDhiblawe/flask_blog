...
def add_user():
    name = None
    form = UsersForm()
    our_users = Users.query.order_by(Users.date_added).all()  # Ensure `our_users` is always defined

    if form.validate_on_submit():
        user = Users.query.filter_by(email=form.email.data).first()
        if user is None:
            user = Users(name=form.name.data, age=form.age.data, email=form.email.data)
            db.session.add(user)
            db.session.commit()
